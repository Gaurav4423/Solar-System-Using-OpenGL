...
void drawSun(GLfloat position[]) {
	...
	glPushMatrix(); 
	glRotatef((GLfloat)z, 1.0, 1.0, 1.0);//(angle,x,y,z)
	glLightfv(GL_LIGHT0, GL_POSITION, position); 
	glDisable(GL_LIGHTING);
	glutSolidSphere(0.8, 40, 16); //(radius,slice(subdivion of z longitude),stack(latitude))
	glPopMatrix();
	glPushMatrix();
	glLightfv(GL_LIGHT0, GL_POSITION, position);
	glDisable(GL_LIGHTING);
	...
	glutSolidTorus(0.2, 0.9, 6, 20);//ring(inner r,outer r,color variation,division of rings)
	glPopMatrix();
}
void drawMercury() {
	glPushMatrix();
	glRotatef((GLfloat)M, 0.0, 1.0, 0.0);//(angle,x,y,z)
...
void drawSaturnWithRings() {
	...
	int i = 0;
	glBegin(GL_QUAD_STRIP);//inner and outer circle 
	for (i = 0; i <= 360; i++)
	{
